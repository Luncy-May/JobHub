...
export async function POST(request) { // json file: { jobname: searchContent } 
    ...
    try {
        ...
        if (closeResult.rows[0].status) { // the job is already opened 
            return Response.json({
                success: false,
                message: "This job has already been opened",
            });
        }
        const query = {
            text: 'UPDATE jobs SET status = $1 WHERE jobid = $2',
            values: [true, jobid], // Use % to match any characters before or after the jobname
        };
        ...
        try {
            return Response.json({
                success: true,
                message: "Successfully open the job!",
                data: data
            });
        } catch (error) {
            return Response.json({
